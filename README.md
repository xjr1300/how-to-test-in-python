...
- [テストの考え方と実装方法 in Python](#テストの考え方と実装方法-in-python)
  ...
  - [pythonにおけるテストの実装](#pythonにおけるテストの実装)
    ...
    - [プロダクションコードとテストの実装方法](#プロダクションコードとテストの実装方法)
      ...
      - [パッチの説明](#パッチの説明)
        - [unittest.mock.patchの使用方法](#unittestmockpatchの使用方法)
  ...
  - [単体テストの実装演習](#単体テストの実装演習)
    ...
    - [テストの実装を演習するサンプルプロジェクトの説明](#テストの実装を演習するサンプルプロジェクトの説明)
...
モックしても、他のテストに影響を与えないようにするためには、モックした後、モックが必要なくなった時点で、モックを解除する必要があります。
...
### テストの実装を演習するサンプルプロジェクトの説明

ドラッグストアの`ドメインルール`は次のとおりです。

> ドメインルールのことを`ビジネスルール`、`ドメインの不変条件(invariant conditions)`と呼びます。
...
- 売上は、異なる売上IDと商品IDの組みをもつ売上明細を複数持ちます。
  - 売上には、同じ商品の売上明細があってはなりません。
...
class Item:
    ...
    def __init__(self, id: uuid.UUID, name: str, unit_price: Decimal) -> None:
        ...
        TODO: 次の単体テストを実装すること
        - 妥当な属性で商品をインスタンス化できることを確認
        - 商品名の先頭の空白文字を削除した商品名でインスタンス化されることを確認
        - 商品名の末尾の空白文字を削除した商品名でインスタンス化されることを確認
        - 商品名の先頭と末尾の空白文字を削除した商品名でインスタンス化されることを確認
        - 空文字の商品名("")で商品をインスタンス化できないことを確認
        - 空白文字だけの商品名("  "など)で商品をインスタンス化できないことを確認
        - 商品名の文字数が範囲外のときに、インスタンス化できないことを確認
            - 商品名の文字列の長さは1文字以上40文字以下とする
        - 単価が範囲外のときに、インスタンス化できないことを確認
            - 単価の範囲は0以上から99,999,999以下とする
        """  # noqa: E501
```
...
イニシャライザでは、売上ID、顧客、売上日時、消費税率を引数で受け取り、メンバ変数に設定します。
