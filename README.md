...
- 消費税率の終点日時が定まらない場合は、pythonの`datetime.max`を使用します。
- 売上の合計額は、小計から割引額を引き、消費税額を加えた額です。
- 売上明細の単価と数量は、0以上の整数です。
