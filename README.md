...
- [テストの考え方と実装方法 in Python](#テストの考え方と実装方法-in-python)
  ...
  - [テストの実装練習](#テストの実装練習)
    ...
    - [消費税率を管理するクラスと単体テストを実装](#消費税率を管理するクラスと単体テストを実装)
      - [消費税の起点日時を変更する場合](#消費税の起点日時を変更する場合)
      - [消費税の終点日時を変更する場合](#消費税の終点日時を変更する場合)
      - [消費税率を追加する場合](#消費税率を追加する場合)
...
- 売上の消費税率は、売上日時が消費税率の起点日時と終点日時で表現される左閉右開区間 ($起点日時 <= 売上日時 < 終点日時$)に含まれる消費税率を適用します。
- 売上の合計額は、小計から割引額を引き、消費税額を加えた額です。
- 顧客、会員区分、売上、売上明細、商品、消費税率は、データベースに保存されます。
...
### 消費税率を管理するクラスと単体テストを実装

消費税率を管理するクラスとその単体テストを実装してください。
消費税率を管理するクラスは、`drugstore/utils/consumption_tax_manager.py`に実装された`ConsumptionTaxManager`クラスです。

`ConsumptionTaxManager`クラスは、最も古い起点日時から最も新しい終点日時まで、期間が途切れないように消費税を管理します。
また、売上日時を基準に、売上に適用する消費税の税率を決定します。

消費税率は、起点日時が左閉区間、終点日時が右開区間で表現されるため、消費税率を適用するときは、売上日時が消費税率の起点日時と終点日時で表現される左閉右開区間 ($起点日時 <= 売上日時 < 終点日時$)に含まれる消費税率を適用します。

単体テストでは、プロセス外依存となるデータベースをモックに置き換えるため、すべての消費税を返すスタブを使用する必要があります。

#### 消費税の起点日時を変更する場合

変更する消費税よりも前の消費税があった場合、前の消費税の終点日時を変更する消費税の起点日時にします。
もし、前の消費税の起点日時が変更する消費税の終点日時以下の場合、前の消費税を削除します。
これは、さらに前の消費税に対しても処理ます。

変更する消費税よりも前の消費税がない場合、変更する消費税の起点日時を`MIN_CONSUMPTION_TAX_BEGIN`に変更します。

#### 消費税の終点日時を変更する場合

変更する消費税よりも後の消費税があった場合、後の消費税の起点日時を変更する消費税の終点日時にします。
もし、後の消費税の終点日時が変更する消費税の終点日時以上の場合、後の消費税を削除します。
これはさらに後の消費税に対しても処理します。

変更する消費税よりも後の消費税がない場合、変更する消費税の終点日時を`MAX_CONSUMPTION_TAX_END`にします。
...
売上クラスのイニシャライザとそのイニシャライザの単体テストを実装してください。
